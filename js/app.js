...
(function(){
  ...
  function v24GenreOverrideHTML(r){
    ...
    return `<div class="override-row"><div style="flex:1;min-width:220px"><strong>Manual override</strong><div class="genre-override-grid">${buttons}</div><label class="genre-select-fallback"><span class="muted small">Fallback list</span><select id="genreOverrideSelect">${opts}</select></label></div><button class="btn btn-secondary" data-action="applyGenreOverride">Apply</button></div><p class="muted small">Use this if Photo Scout gets the genre wrong during beta testing. The icons now use a consistent professional SVG style rather than emoji.</p>`;
  }
  ...
  function v24DetectionExplanationHTML(r){
    const subjects=(r.subjects||[]).map(x=>`<span>${escapeHTML(x)}</span>`).join('');
    const notes=(r.analysisNotes||[]).map(x=>`<li>${escapeHTML(x)}</li>`).join('')||'<li>No very strong visual cue detected; treat this as a beta suggestion.</li>';
    const candidates=(r.candidates||[]).map(c=>`<span>${genreIconSVG(c.genre)} ${label(c.genre)} ${c.score}</span>`).join('');
    return `<p class="muted">Detected as <strong>${escapeHTML(r.genre.name)}</strong> because of the visible image cues below. This is local browser analysis only; no image upload is used in this build.</p><div class="mini-chips">${subjects}</div><ul class="reason-list">${notes}</ul><p class="muted small">Alternative matches: ${candidates||'none'}</p>`;
  }
  const oldRender=renderAnalysis;
  renderAnalysis=function(){
  ...
  function patchHelp(){
    try{renderHelp(); const c=$('#helpTab .content'); if(c&&!$('#v24HelpCard')){const card=document.createElement('div');card.className='card help-section';card.id='v24HelpCard';card.innerHTML=`<h3>${iconSVG('settings')} v24 Icon Polish</h3><p>The Scene Analysis and Manual Override genre options now use professional SVG icons for Landscape, Wildlife, Night, Fireworks, Portrait, Macro, Street and Architecture.</p>`;c.appendChild(card)}}catch{}
  }
