...
(function(){
  ...
  const APP_VERSION_LABEL='Beta v26';
  const VERSION_URL='version.json';
  const WHATS_NEW=[
    'Automatic update detection for beta testers.',
    'Update banner when a newer build is published.',
    'Stronger cache refresh handling for iPhone/Android testing.',
    'Visible app version number for support and tester feedback.',
    'What’s New screen after each major update.'
  ];
  ...
  function patchHelpWithVersion(){
    try{
      const content=qs('#helpTab .content'); if(!content || qs('#v26VersionCard')) return;
      const card=document.createElement('div'); card.id='v26VersionCard'; card.className='card help-section version-card';
  ...
  function bind(){
    setVersionLabels();
    const apply=qs('#applyUpdateBtn'); if(apply&&!apply.dataset.v26){apply.dataset.v26='1'; apply.addEventListener('click',()=>applyUpdate('26'));}
    const dismiss=qs('#dismissUpdateBtn'); if(dismiss&&!dismiss.dataset.v26){dismiss.dataset.v26='1'; dismiss.addEventListener('click',hideUpdateBanner);}
