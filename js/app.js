...
function init(){setTimeout(()=>$('#splashScreen').classList.add('hidden'),5000);tickClock();setInterval(tickClock,1000);bindUI();renderHelp();renderPlanner();updateCount();initCamera();if(!localStorage.getItem('photoScout.tutorialDone'))showTutorial();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js?v=31').catch(()=>{});}
...
// v26: automatic update/version management for beta testing.
(function(){
  const APP_VERSION='31.0.0';
  const APP_VERSION_LABEL='Beta v31';
  const VERSION_URL='version.json';
  const WHATS_NEW=[
    'Refined update system so the banner does not keep appearing after updating.',
    'Dismiss until next version behaviour added for testers.',
    'Smaller, less intrusive update notification logic.',
    'Improved cache acknowledgement after successful update.',
    'Update history and version display improved for beta support.'
  ];
  ...
  function setVersionLabels(){
    document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>{el.textContent=APP_VERSION_LABEL});
    document.documentElement.dataset.photoScoutVersion=APP_VERSION;
  }
  function showUpdateBanner(remote){
    const banner=qs('#updateBanner'); if(!banner) return;
    const txt=qs('#updateBannerText'); if(txt) txt.textContent=`Latest beta ${remote.version||remote.build||''} is available. Tap update to refresh cached files.`;
    banner.hidden=false;
  }
  function hideUpdateBanner(){const b=qs('#updateBanner'); if(b)b.hidden=true;}
  async function clearAppCaches(){
    try{ if(navigator.serviceWorker?.controller) navigator.serviceWorker.controller.postMessage({type:'CLEAR_PHOTO_SCOUT_CACHES'}); }catch{}
    try{ if(window.caches){ const keys=await caches.keys(); await Promise.all(keys.filter(k=>k.startsWith('photo-scout-')).map(k=>caches.delete(k))); } }catch{}
  }
  async function applyUpdate(version){
    try{localStorage.setItem('photoScout.pendingUpdateApplied',String(version||APP_VERSION));}catch{}
    ...
    location.replace(base+'?v='+(version||APP_VERSION)+'&t='+Date.now());
  }
  async function checkForUpdate(silent=true){
    try{
      ...
      if(remote && isNewer(remote.version,APP_VERSION)){ showUpdateBanner(remote); return remote; }
      if(!silent){ alert('Photo Scout is up to date: '+APP_VERSION_LABEL); }
    }catch(err){ if(!silent) alert('Could not check for updates. Try again when online.'); }
    return null;
  }
  function showWhatsNew(force=false){
    const seen=localStorage.getItem('photoScout.lastSeenVersion');
    if(!force && seen===APP_VERSION) return;
    const dlg=qs('#whatsNewDialog'), body=qs('#whatsNewContent'); if(!dlg||!body) return;
    body.innerHTML=`<h2>Photo Scout ${APP_VERSION_LABEL}</h2><p class="muted">This beta now includes automatic update and cache management, so testers should not normally need a new ?v= link each time.</p><ul class="whats-new-list">${WHATS_NEW.map(x=>`<li>${x}</li>`).join('')}</ul><div class="update-actions"><button class="btn btn-secondary" data-action="checkForUpdate">Check for updates</button><button class="btn btn-secondary" data-action="clearPhotoScoutCache">Clear app cache</button><button class="btn btn-primary" data-action="closeWhatsNew">Continue</button></div><p class="cache-note">If a tester still sees an older version, ask them to close the app and reopen it. The app now checks for updates automatically when opened and when returning online.</p>`;
    try{dlg.showModal();}catch{}
    localStorage.setItem('photoScout.lastSeenVersion',APP_VERSION);
  }
  function patchHelpWithVersion(){
    try{
      const content=qs('#helpTab .content'); if(!content || qs('#v30VersionCard')) return;
      const card=document.createElement('div'); card.id='v30VersionCard'; card.className='card help-section version-card';
      card.innerHTML=`<h3>Photo Scout ${APP_VERSION_LABEL}</h3><p>Automatic update checks are now enabled. The app checks for a newer beta when opened, when returning online, and periodically during testing.</p><div class="update-actions"><button class="btn btn-secondary" data-action="checkForUpdate">Check for updates</button><button class="btn btn-secondary" data-action="showWhatsNew">What’s new</button><button class="btn btn-secondary" data-action="clearPhotoScoutCache">Clear cache</button></div><p class="cache-note">Build: ${APP_VERSION}. If a feature still appears stale, use Clear cache and reopen the app.</p>`;
      content.insertBefore(card, content.firstChild);
      try{photoScoutAttachActions(card)}catch{}
    }catch{}
  }
  async function registerV26ServiceWorker(){
    try{
      if(!('serviceWorker' in navigator)) return;
      const reg=await navigator.serviceWorker.register('sw.js?v=30');
      reg.addEventListener('updatefound',()=>{
        const sw=reg.installing; if(!sw) return;
        sw.addEventListener('statechange',()=>{ if(sw.state==='installed' && navigator.serviceWorker.controller){ showUpdateBanner({version:APP_VERSION,build:'new service worker'}); } });
      });
      navigator.serviceWorker.addEventListener('controllerchange',()=>{ try{localStorage.setItem('photoScout.controllerUpdatedAt',String(Date.now()));}catch{} });
    }catch{}
  }
  const previousAction=window.photoScoutRunAction;
  window.photoScoutRunAction=function(action,id,sourceEvent){
    if(action==='checkForUpdate'){checkForUpdate(false);return true;}
    if(action==='clearPhotoScoutCache'){clearAppCaches().then(()=>alert('Photo Scout cache cleared. Close and reopen the app if needed.'));return true;}
    if(action==='showWhatsNew'){showWhatsNew(true);return true;}
    if(action==='closeWhatsNew'){try{qs('#whatsNewDialog')?.close()}catch{};return true;}
    return previousAction ? previousAction(action,id,sourceEvent) : false;
  };
  function bind(){
    setVersionLabels();
    const apply=qs('#applyUpdateBtn'); if(apply&&!apply.dataset.v30){apply.dataset.v30='1'; apply.addEventListener('click',()=>applyUpdate('30'));}
    const dismiss=qs('#dismissUpdateBtn'); if(dismiss&&!dismiss.dataset.v30){dismiss.dataset.v30='1'; dismiss.addEventListener('click',hideUpdateBanner);}
    registerV26ServiceWorker();
    setTimeout(()=>showWhatsNew(false),1600);
    ...
    setTimeout(patchHelpWithVersion,800); setInterval(patchHelpWithVersion,3500);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',bind); else bind();
})();

// v28: lightweight field technique/filter recommendations.
(function(){
  const V28='28';
  function v28FieldTechniqueSuggestions(r){
    const m=r?.metrics||{}; const g=(r?.genre?.type||'').toLowerCase();
    const suggestions=[];
    const add=(title,body,why)=>suggestions.push({title,body,why});
    const bright=m.brightRatio||0, contrast=m.contrast||0, sky=m.blueSkyRatio||0, green=m.greenVegRatio||0, dark=m.darkRatio||0, sat=m.saturation||0;
    if((bright>.09 && (sky>.04 || g==='landscape')) || (contrast>.25 && sky>.07)){
      add('Graduated ND / exposure bracketing suggested','The scene appears to contain a brighter upper/sky region compared with the rest of the frame. Consider a soft graduated ND filter, exposure bracketing, or returning when the light is softer.','Bright sky / high dynamic range cues detected.');
    }
    if((sky>.10 && sat>.16) || (g==='landscape' && (sky>.06 || green>.12))){
      add('Circular polariser may help','For blue sky, foliage, water, glass or reflective surfaces, a circular polariser can deepen sky contrast, reduce glare and improve colour separation. Rotate the filter while viewing for the strongest effect.','Sky/foliage/reflection-prone scene cues detected.');
    }
    if(g==='landscape' && contrast>.22){
      add('Consider HDR or exposure blending','If the sky and foreground are difficult to balance, shoot a bracketed sequence for later blending in Lightroom/Photoshop.','Landscape with elevated contrast.');
    }
    if(g==='night' || dark>.34){
      add('Tripod and remote release recommended','Low light will benefit from tripod support, a self-timer/remote release and image stabilisation switched off when mounted.','Low-light/dark-frame cues detected.');
    }
    if(g==='macro'){
      add('Diffuser / reflector recommended','For close-up work, soft diffused light usually improves texture and reduces harsh specular highlights.','Close-up/detail scene detected.');
    }
    if(g==='wildlife'){
      add('Fast shutter / telephoto planning','For wildlife locations, plan for a long lens, fast shutter speed and sufficient ISO headroom rather than relying on phone preview brightness.','Wildlife/natural texture cues detected.');
    }
    if(!suggestions.length){
      add('Field technique check','Review sky brightness, reflections, tripod need and whether a polariser or graduated filter would improve the return visit.','No strong filter-specific cue detected.');
    }
    return suggestions.slice(0,4);
  }
  function v28TechniqueHTML(r){
    const s=v28FieldTechniqueSuggestions(r);
    return `<div class="card" id="v28TechniqueCard"><h3>${iconSVG('tips')} Field Technique / Filter Suggestions</h3><p class="muted small">Lightweight recommendations only — no live filter simulation is applied, so this should not affect camera performance.</p>${s.map(x=>`<div class="tip"><strong>${escapeHTML(x.title)}</strong><br>${escapeHTML(x.body)}<br><span class="muted small">${escapeHTML(x.why)}</span></div>`).join('')}</div>`;
  }
  const oldRender=renderAnalysis;
  renderAnalysis=function(){
    oldRender();
    const content=$('#analysisView .content');
    if(content && !$('#v28TechniqueCard')){
      const holder=document.createElement('div'); holder.innerHTML=v28TechniqueHTML(currentRecommendations);
      const lighting=[...content.children].find(el=>el.textContent&&el.textContent.includes('Lighting'));
      if(lighting && lighting.nextSibling) content.insertBefore(holder.firstElementChild, lighting.nextSibling); else content.appendChild(holder.firstElementChild);
    }
  };
  const oldReport=locationReportText;
  locationReportText=function(i){
    const base=oldReport(i); const s=v28FieldTechniqueSuggestions(i.recommendations||{});
    return [base,'','Field technique / filter suggestions:',...s.map(x=>`- ${x.title}: ${x.body}`)].join('\n');
  };
  try{const pill=document.getElementById('appVersionPill'); if(pill) pill.textContent='Beta v28';}catch{}
})();


// v30: refined update prompt acknowledgement + sat-nav/location suggestion integration.
(function(){
  const APP_VERSION_29='31.0.0';
  const APP_LABEL_29='Beta v31';
  // Prevent the older v26 automatic What's New dialog from reappearing after users have already updated.
  try{localStorage.setItem('photoScout.lastSeenVersion','26.0.0');localStorage.setItem('photoScout.lastSeenVersion.v30',APP_VERSION_29);}catch{}
  function q(sel){return document.querySelector(sel)}
  function patchVersionLabels(){
    document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=APP_LABEL_29);
    document.documentElement.dataset.photoScoutVersion=APP_VERSION_29;
  }
  function silenceOldUpdatePrompts(){
    const dlg=q('#whatsNewDialog');
    if(dlg && dlg.open){try{dlg.close()}catch{}}
    if(dlg) dlg.classList.add('auto-suppressed');
    const banner=q('#updateBanner');
    if(banner) banner.hidden=true;
  }
  function coordsFrom(obj){
    if(!obj) return null;
    if(obj.coords && typeof obj.coords.lat==='number' && typeof obj.coords.lng==='number') return {lat:obj.coords.lat,lng:obj.coords.lng,name:obj.name||'Photo Scout Location'};
    if(typeof obj.lat==='number' && typeof obj.lng==='number') return {lat:obj.lat,lng:obj.lng,name:obj.name||'Photo Scout Location'};
    return null;
  }
  function enc(v){return encodeURIComponent(v||'Photo Scout Location')}
  function navURL(lat,lng,name,app){
    const label=enc(name);
    if(app==='apple') return `https://maps.apple.com/?daddr=${lat},${lng}&q=${label}`;
    if(app==='google') return `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&destination_place_id=&travelmode=driving`;
    if(app==='waze') return `https://waze.com/ul?ll=${lat},${lng}&navigate=yes`;
    return mapURL(lat,lng);
  }
  function openNav(lat,lng,name,app){
    const url=navURL(Number(lat),Number(lng),name,app);
    window.open(url,'_blank','noopener');
  }
  function navButtons(lat,lng,name){
    return `<div class="card nav-card"><h3>${iconSVG('map')} Navigate / Sat Nav</h3><p class="muted small">Open this location in your preferred navigation app. Works from saved Photo Scout locations and suggested UK photography locations.</p><div class="nav-choice-grid"><button class="btn btn-secondary" data-action="navigateApple" data-lat="${lat}" data-lng="${lng}" data-name="${escapeHTML(name)}">Apple Maps</button><button class="btn btn-secondary" data-action="navigateGoogle" data-lat="${lat}" data-lng="${lng}" data-name="${escapeHTML(name)}">Google Maps</button><button class="btn btn-secondary" data-action="navigateWaze" data-lat="${lat}" data-lng="${lng}" data-name="${escapeHTML(name)}">Waze</button></div></div>`;
  }
  const UK_LOCATIONS=[
    {name:'Beachy Head & Seven Sisters',lat:50.7397,lng:0.2409,region:'East Sussex',type:'coastal landscape',goodFor:['Landscape','Coastal','Golden hour','Long exposure'],notes:'Cliffs, sea views and dramatic weather. Take care near cliff edges.'},
    {name:'Ditchling Beacon',lat:50.9015,lng:-0.1077,region:'West Sussex',type:'landscape viewpoint',goodFor:['Landscape','Sunrise','Telephoto layers'],notes:'High viewpoint across the South Downs, useful for sunrise and layered landscape work.'},
    {name:'Arundel Castle & River Arun',lat:50.8560,lng:-0.5548,region:'West Sussex',type:'architecture/landscape',goodFor:['Architecture','Landscape','Street','Blue hour'],notes:'Castle, town details and river viewpoints in one compact area.'},
    {name:'Brighton Palace Pier & Seafront',lat:50.8195,lng:-0.1368,region:'East Sussex',type:'urban/coastal',goodFor:['Street','Architecture','Seascape','Night'],notes:'Strong for street, night, pier details and long-exposure seafront work.'},
    {name:'Ashdown Forest',lat:51.0660,lng:0.0330,region:'East Sussex',type:'woodland/heathland',goodFor:['Woodland','Landscape','Wildlife','Autumn'],notes:'Heathland, woodland, mist and seasonal detail work.'},
    {name:'RSPB Pulborough Brooks',lat:50.9519,lng:-0.5132,region:'West Sussex',type:'wildlife reserve',goodFor:['Wildlife','Birds','Telephoto','Dawn'],notes:'Wildlife and bird photography; check opening hours and hides before travelling.'},
    {name:'Rye Harbour Nature Reserve',lat:50.9359,lng:0.7582,region:'East Sussex',type:'wildlife/coastal',goodFor:['Wildlife','Coastal','Minimalist','Big skies'],notes:'Birds, wetland, shingle and big-sky coastal compositions.'},
    {name:'London South Bank',lat:51.5066,lng:-0.1163,region:'London',type:'urban/street',goodFor:['Street','Architecture','Night','Reflections'],notes:'Urban layers, people, bridges, reflections and blue-hour city scenes.'},
    {name:'Bodiam Castle',lat:51.0023,lng:0.5435,region:'East Sussex',type:'architecture/heritage',goodFor:['Architecture','Landscape','Reflections','Mist'],notes:'Moated castle with strong reflection opportunities in calm conditions.'},
    {name:'Wakehurst',lat:51.0694,lng:-0.0895,region:'West Sussex',type:'woodland/macro',goodFor:['Macro','Woodland','Flowers','Seasonal colour'],notes:'Excellent for seasonal colour, detail work and macro subjects.'}
  ];
  ...
  function userLatLng(){
    try{if(currentPosition?.coords) return {lat:currentPosition.coords.latitude,lng:currentPosition.coords.longitude};}catch{}
    return null;
  }
  function suggestedLocationsHTML(){
    const u=userLatLng();
    const genre=(currentRecommendations?.genre?.name||'').toLowerCase();
    const ranked=UK_LOCATIONS.map(loc=>{
      const dist=u?distanceMiles(u.lat,u.lng,loc.lat,loc.lng):null;
      let score=0;
      const hay=(loc.type+' '+loc.goodFor.join(' ')).toLowerCase();
      if(genre && hay.includes(genre)) score+=5;
      if(genre==='landscape' && /coastal|viewpoint|landscape/.test(hay)) score+=3;
      if(genre==='wildlife' && /wildlife|birds/.test(hay)) score+=4;
      if(genre==='architecture' && /castle|architecture|heritage|urban/.test(hay)) score+=4;
      if(genre==='street' && /street|urban|london|brighton/.test(hay)) score+=4;
      if(genre==='macro' && /macro|woodland|flowers/.test(hay)) score+=4;
      if(dist!==null) score+=Math.max(0,4-Math.min(dist/25,4));
      return {...loc,dist,score};
    }).sort((a,b)=>b.score-a.score).slice(0,5);
    return `<div class="card" id="v29SuggestedLocations"><h3>${iconSVG('map')} Suggested UK Photography Locations</h3><p class="muted small">UK-first beta database. These are curated starter suggestions, not a complete national location library. Tap a sat-nav option to plan a visit.</p><div class="suggested-location-list">${ranked.map(loc=>`<div class="suggested-location-card"><h4>${escapeHTML(loc.name)}</h4><p class="muted small">${escapeHTML(loc.region)} · ${escapeHTML(loc.type)}${loc.dist!==null?` · approx. ${loc.dist.toFixed(1)} miles`:''}</p><div class="mini-chips">${loc.goodFor.map(x=>`<span>${escapeHTML(x)}</span>`).join('')}</div><p class="small">${escapeHTML(loc.notes)}</p><div class="suggested-actions"><button class="btn btn-secondary" data-action="navigateApple" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${escapeHTML(loc.name)}">Apple Maps</button><button class="btn btn-secondary" data-action="navigateGoogle" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${escapeHTML(loc.name)}">Google Maps</button><button class="btn btn-secondary" data-action="navigateWaze" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${escapeHTML(loc.name)}">Waze</button></div></div>`).join('')}</div></div>`;
  }
  // Patch saved-location detail to add explicit navigation choices.
  const oldShowDetail=window.showDetail||showDetail;
  showDetail=window.showDetail=function(id){
    oldShowDetail(id);
    const item=PhotoScoutStorage.get().find(x=>x.id===id); const c=coordsFrom(item);
    const content=q('#detailContent');
    if(c && content && !q('#v29NavCard')){
      const holder=document.createElement('div'); holder.id='v29NavCard'; holder.innerHTML=navButtons(c.lat,c.lng,c.name);
      const locCard=[...content.querySelectorAll('.card')].find(el=>el.textContent&&el.textContent.includes('Location'));
      if(locCard && locCard.nextSibling) content.insertBefore(holder.firstElementChild,locCard.nextSibling); else content.appendChild(holder.firstElementChild);
      try{photoScoutAttachActions(content)}catch{}
    }
  };
  // Patch planner to include suggested UK locations and navigation buttons.
  const oldRenderPlanner=window.renderPlanner||renderPlanner;
  renderPlanner=window.renderPlanner=function(){
    oldRenderPlanner();
    const content=q('#plannerTab .content');
    if(content && !q('#v29SuggestedLocations')){
      const wrap=document.createElement('div'); wrap.innerHTML=suggestedLocationsHTML();
      content.appendChild(wrap.firstElementChild);
      try{photoScoutAttachActions(content)}catch{}
    }
  };
  // Add a compact navigate button to planner cards when GPS is saved.
  const oldRenderLocations=window.renderLocations||renderLocations;
  renderLocations=window.renderLocations=function(){
    oldRenderLocations();
    try{photoScoutAttachActions(q('#locationsTab')||document)}catch{}
  };
  const prior=window.photoScoutRunAction;
  window.photoScoutRunAction=function(action,id,e){
    const el=e?.target?.closest?.('[data-action]');
    const lat=Number(el?.dataset?.lat), lng=Number(el?.dataset?.lng), name=el?.dataset?.name||'Photo Scout Location';
    if(action==='navigateApple' && Number.isFinite(lat) && Number.isFinite(lng)){openNav(lat,lng,name,'apple');return true;}
    if(action==='navigateGoogle' && Number.isFinite(lat) && Number.isFinite(lng)){openNav(lat,lng,name,'google');return true;}
    if(action==='navigateWaze' && Number.isFinite(lat) && Number.isFinite(lng)){openNav(lat,lng,name,'waze');return true;}
    if(action==='showWhatsNew'){ // keep available from Help only, but do not auto-show on open.
      const dlg=q('#whatsNewDialog'); const body=q('#whatsNewContent');
      if(body){body.innerHTML=`<h2>Photo Scout ${APP_LABEL_29}</h2><ul class="whats-new-list"><li>Update banner acknowledgement refined so it does not keep appearing after updating.</li><li>Dismiss until next version logic improved.</li><li>Sat-nav route planning retained for Apple Maps, Google Maps and Waze.</li><li>UK-first suggested photography locations retained in Planner.</li></ul><div class="update-actions"><button class="btn btn-primary" data-action="closeWhatsNew">Continue</button></div>`;}
      try{dlg?.classList.remove('auto-suppressed');dlg?.showModal();}catch{}
      return true;
    }
    return prior?prior(action,id,e):false;
  };
  function bindV29(){
    patchVersionLabels();
    silenceOldUpdatePrompts();
    setTimeout(silenceOldUpdatePrompts,1700);
    setTimeout(()=>{try{renderPlanner()}catch{}},700);
    const apply=q('#applyUpdateBtn'); if(apply){apply.onclick=async()=>{try{if(window.caches){const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('photo-scout-')).map(k=>caches.delete(k)));}}catch{} location.replace(location.href.split('?')[0]+'?v=30&t='+Date.now());};}
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',bindV29); else bindV29();
})();


// v30: final update-system refinement to prevent repeated update-banner loop.
(function(){
  const V='31.0.0', LABEL='Beta v31';
  const ACK_KEY='photoScout.updateAcknowledgedVersion';
  const DISMISS_KEY='photoScout.updateDismissedVersion';
  const LAST_SEEN_KEY='photoScout.lastSeenVersion';
  function q(s){return document.querySelector(s)}
  function setLabels(){
    document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>{el.textContent=LABEL});
    document.documentElement.dataset.photoScoutVersion=V;
  }
  function acknowledge(version=V){
    try{
      localStorage.setItem(ACK_KEY,version);
      localStorage.setItem(DISMISS_KEY,version);
      localStorage.setItem(LAST_SEEN_KEY,version);
      localStorage.setItem('photoScout.lastSeenVersion.v30',version);
      localStorage.removeItem('photoScout.pendingUpdateApplied');
    }catch{}
  }
  function hideBanner(){const b=q('#updateBanner'); if(b){b.hidden=true; b.classList.add('v30-hidden');}}
  function showSubtleUpdateDot(version){
    try{
      const btn=q('#settingsBtn'); if(!btn) return;
      btn.classList.add('has-update');
      btn.title='Photo Scout update available';
      btn.dataset.updateVersion=version||'';
    }catch{}
  }
  function clearSubtleUpdateDot(){try{q('#settingsBtn')?.classList.remove('has-update')}catch{}}
  async function clearCaches(){
    try{ if(navigator.serviceWorker?.controller) navigator.serviceWorker.controller.postMessage({type:'CLEAR_PHOTO_SCOUT_CACHES'}); }catch{}
    try{ if(window.caches){ const keys=await caches.keys(); await Promise.all(keys.filter(k=>k.startsWith('photo-scout-')).map(k=>caches.delete(k))); } }catch{}
  }
  async function applyV30Update(version=V){
    acknowledge(version);
    hideBanner(); clearSubtleUpdateDot();
    await clearCaches();
    try{ if('serviceWorker' in navigator){ const regs=await navigator.serviceWorker.getRegistrations(); await Promise.all(regs.map(r=>r.update().catch(()=>{}))); } }catch{}
    const base=location.href.split('?')[0].split('#')[0];
    location.replace(base+'?v=30&t='+Date.now());
  }
  function shouldSuppress(remoteVersion){
    try{
      if(!remoteVersion || String(remoteVersion)===V) return true;
      if(localStorage.getItem(ACK_KEY)===String(remoteVersion)) return true;
      if(localStorage.getItem(DISMISS_KEY)===String(remoteVersion)) return true;
    }catch{}
    return false;
  }
  function bindUpdateButtons(){
    const apply=q('#applyUpdateBtn');
    if(apply){ apply.onclick=()=>applyV30Update(apply.dataset.remoteVersion||V); }
    const dismiss=q('#dismissUpdateBtn');
    if(dismiss){ dismiss.onclick=()=>{ const v=apply?.dataset?.remoteVersion||V; try{localStorage.setItem(DISMISS_KEY,v)}catch{}; hideBanner(); showSubtleUpdateDot(v); }; }
  }
  // Watch the older update system. If it shows a banner for the current version, hide it permanently.
  function watchBanner(){
    const b=q('#updateBanner'); if(!b) return;
    const apply=q('#applyUpdateBtn');
    const txt=q('#updateBannerText');
    const process=()=>{
      setLabels();
      const m=(txt?.textContent||'').match(/(\d+\.\d+\.\d+)/);
      const remote=m?m[1]:V;
      if(apply) apply.dataset.remoteVersion=remote;
      if(!b.hidden && shouldSuppress(remote)){hideBanner(); clearSubtleUpdateDot();}
    };
    process(); bindUpdateButtons();
    new MutationObserver(()=>{process(); bindUpdateButtons();}).observe(b,{attributes:true,childList:true,subtree:true,attributeFilter:['hidden','class']});
  }
  const prior=window.photoScoutRunAction;
  window.photoScoutRunAction=function(action,id,e){
    if(action==='checkForUpdate'){
      fetch('version.json?t='+Date.now(),{cache:'no-store'}).then(r=>r.json()).then(remote=>{
        const rv=remote.version||V;
        if(rv===V){acknowledge(V); hideBanner(); clearSubtleUpdateDot(); alert('Photo Scout is up to date: '+LABEL);}
        else if(shouldSuppress(rv)){showSubtleUpdateDot(rv); alert('A newer build was already dismissed. Use What’s New / Update Tools if needed.');}
        else {showSubtleUpdateDot(rv); alert('New Photo Scout beta available: '+rv);}
      }).catch(()=>alert('Could not check for updates. Try again when online.'));
      return true;
    }
    if(action==='showWhatsNew'){
      const dlg=q('#whatsNewDialog'), body=q('#whatsNewContent');
      if(body){body.innerHTML='<h2>Photo Scout '+LABEL+'</h2><ul class="whats-new-list"><li>Update banner now acknowledges the installed version properly.</li><li>Dismiss until next version is supported.</li><li>Repeated update prompt loop reduced.</li><li>Sat-nav integration and UK suggested locations retained.</li></ul><div class="update-actions"><button class="btn btn-secondary" data-action="checkForUpdate">Check for updates</button><button class="btn btn-secondary" data-action="clearPhotoScoutCache">Clear cache</button><button class="btn btn-primary" data-action="closeWhatsNew">Continue</button></div>';}
      try{dlg?.classList.remove('auto-suppressed'); dlg?.showModal(); acknowledge(V);}catch{}
      return true;
    }
    if(action==='clearPhotoScoutCache'){clearCaches().then(()=>{acknowledge(V); hideBanner(); alert('Photo Scout cache cleared. Reopen the app if anything looks stale.');});return true;}
    return prior?prior(action,id,e):false;
  };
  function init(){setLabels(); acknowledge(V); hideBanner(); bindUpdateButtons(); watchBanner(); setTimeout(()=>{hideBanner();},3000);}
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',init); else init();
})();

// v31: expanded UK location database, user-added locations, clearer nav + live weather panel.
(function(){
  const V='31.0.0';
  const LABEL='Beta v31';
  const CUSTOM_KEY='photoScout.customSuggestedLocations.v1';
  const WEATHER_KEY='photoScout.lastLiveWeather.v1';
  ...
  function enc(v){return encodeURIComponent(v||'Photo Scout Location')}
  function mi(a,b,c,d){const R=3958.8,rad=x=>x*Math.PI/180;const dLat=rad(c-a),dLng=rad(d-b);const h=Math.sin(dLat/2)**2+Math.cos(rad(a))*Math.cos(rad(c))*Math.sin(dLng/2)**2;return R*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h));}
  function userLL(){try{if(window.currentPosition?.coords)return{lat:currentPosition.coords.latitude,lng:currentPosition.coords.longitude}}catch{} return null;}
  function icon(name){try{return iconSVG(name)}catch{return ''}}
  function codeSummary(c){const map={0:'Clear',1:'Mainly clear',2:'Partly cloudy',3:'Overcast',45:'Fog',48:'Rime fog',51:'Light drizzle',53:'Drizzle',55:'Heavy drizzle',61:'Light rain',63:'Rain',65:'Heavy rain',71:'Light snow',73:'Snow',75:'Heavy snow',80:'Rain showers',81:'Showers',82:'Heavy showers',95:'Thunderstorm'};return map[c]||'Forecast';}
  function navURL(lat,lng,name,app){if(app==='apple')return`https://maps.apple.com/?daddr=${lat},${lng}&q=${enc(name)}`; if(app==='google')return`https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving`; if(app==='waze')return`https://waze.com/ul?ll=${lat},${lng}&navigate=yes`; return`https://www.google.com/maps/search/?api=1&query=${lat},${lng}`;}
  function openNav(lat,lng,name,app){window.open(navURL(Number(lat),Number(lng),name,app),'_blank','noopener')}
  const CURATED=[
    ['Durdle Door',50.6211,-2.2768,'Dorset','coastal landscape',['Landscape','Coastal','Sunrise','Milky Way','Long exposure'],'Iconic limestone arch and Jurassic Coast views. Best at sunrise/blue hour; check access, parking and cliff safety.'],
    ['Lulworth Cove',50.6185,-2.2478,'Dorset','coastal landscape',['Landscape','Coastal','Seascape','Golden hour'],'Curved cove, cliff-top views and nearby access to Durdle Door.'],
    ['Old Harry Rocks',50.6423,-1.9235,'Dorset','coastal landscape',['Landscape','Coastal','Sunrise','Drone-style viewpoints'],'Chalk stacks and sweeping coastline; good for sunrise and wide-angle compositions.'],
    ['Portland Bill Lighthouse',50.5145,-2.4576,'Dorset','coastal lighthouse',['Landscape','Coastal','Long exposure','Night'],'Lighthouse, rocks and dramatic sea conditions. Strong long-exposure location.'],
    ['Seven Sisters & Cuckmere Haven',50.7607,0.1480,'East Sussex','coastal cliffs',['Landscape','Coastal','Golden hour','Long exposure'],'Classic chalk-cliff composition. Take care near cliff edges and tides.'],
    ['Beachy Head',50.7397,0.2409,'East Sussex','coastal cliffs',['Landscape','Coastal','Minimalist','Telephoto'],'High chalk cliffs, lighthouse views and dramatic weather.'],
    ['Brighton Palace Pier',50.8195,-0.1368,'East Sussex','urban coastal',['Street','Architecture','Night','Seascape'],'Pier, seafront and night photography. Good for long-exposure and street.'],
    ['Ditchling Beacon',50.9015,-0.1077,'West Sussex','landscape viewpoint',['Landscape','Sunrise','Telephoto layers','South Downs'],'High viewpoint across the South Downs.'],
    ['Arundel Castle & River Arun',50.8560,-0.5548,'West Sussex','architecture heritage',['Architecture','Landscape','Blue hour','Reflections'],'Castle, town details and river viewpoints.'],
    ['Bodiam Castle',51.0023,0.5435,'East Sussex','architecture heritage',['Architecture','Landscape','Reflections','Mist'],'Moated castle; strong reflection opportunities.'],
    ['Rye Harbour Nature Reserve',50.9359,0.7582,'East Sussex','wildlife coastal',['Wildlife','Birds','Coastal','Big skies'],'Birds, wetland, shingle and big-sky minimal scenes.'],
    ['RSPB Pulborough Brooks',50.9519,-0.5132,'West Sussex','wildlife reserve',['Wildlife','Birds','Telephoto','Dawn'],'Wetland reserve and bird hides; check opening times.'],
    ['Ashdown Forest',51.0660,0.0330,'East Sussex','woodland heathland',['Woodland','Landscape','Wildlife','Autumn'],'Heathland, woodland, mist and seasonal detail work.'],
    ['Wakehurst',51.0694,-0.0895,'West Sussex','woodland macro',['Macro','Woodland','Flowers','Seasonal colour'],'Strong for macro, flowers, woodland and seasonal colour.'],
    ['New Forest National Park',50.8750,-1.6320,'Hampshire','woodland heathland',['Wildlife','Woodland','Landscape','Autumn'],'Ponies, ancient woodland, mist and heathland.'],
    ['Corfe Castle',50.6395,-2.0566,'Dorset','ruins landscape',['Architecture','Landscape','Mist','Sunrise'],'Ruins, village and misty sunrise opportunities.'],
    ['St Michael’s Mount',50.1179,-5.4775,'Cornwall','coastal heritage',['Landscape','Architecture','Coastal','Blue hour'],'Causeway, island castle and coastal views; check tides.'],
    ['Kynance Cove',49.9740,-5.2300,'Cornwall','coastal landscape',['Landscape','Coastal','Sunset','Seascape'],'Dramatic rocks, turquoise water and coastal compositions.'],
    ['Land’s End',50.0660,-5.7147,'Cornwall','coastal landscape',['Landscape','Coastal','Sunset','Storm light'],'Cliff/coastal compositions and sunset potential.'],
    ['Glastonbury Tor',51.1445,-2.6992,'Somerset','landscape landmark',['Landscape','Sunrise','Mist','Telephoto'],'Iconic hilltop tower, mist and layered Somerset views.'],
    ['Stonehenge',51.1789,-1.8262,'Wiltshire','heritage landscape',['Architecture','Landscape','Sunrise','Silhouette'],'Iconic heritage site; access/photography restrictions may apply.'],
    ['Avebury Stone Circle',51.4289,-1.8540,'Wiltshire','heritage landscape',['Landscape','Heritage','Golden hour','Wide angle'],'Large stone circle with village context and landscape opportunities.'],
    ['London South Bank',51.5066,-0.1163,'London','urban street',['Street','Architecture','Night','Reflections'],'Street, bridges, people, blue-hour skyline and reflections.'],
    ['Tower Bridge',51.5055,-0.0754,'London','architecture landmark',['Architecture','Blue hour','Night','Reflections'],'Classic London landmark with strong blue-hour potential.'],
    ['Canary Wharf',51.5054,-0.0235,'London','modern architecture',['Architecture','Street','Night','Reflections'],'Glass, symmetry, reflections and urban night photography.'],
    ['St Paul’s / Millennium Bridge',51.5138,-0.0984,'London','architecture cityscape',['Architecture','Street','Blue hour','Long exposure'],'Leading lines, cityscape and classic London framing.'],
    ['Richmond Park',51.4440,-0.2740,'London','wildlife park',['Wildlife','Landscape','Mist','Telephoto'],'Deer, misty mornings and open parkland. Keep safe distance from wildlife.'],
    ['Wistman’s Wood',50.5787,-3.9603,'Devon','ancient woodland',['Woodland','Macro','Mist','Atmosphere'],'Mossy ancient woodland; tread carefully and avoid damaging habitat.'],
    ['Dartmoor Haytor',50.5808,-3.7555,'Devon','moorland landscape',['Landscape','Astro','Sunrise','Telephoto'],'Granite tor, moorland layers and night-sky potential.'],
    ['Exmoor Valley of Rocks',51.2296,-3.8317,'Devon','coastal landscape',['Landscape','Coastal','Wildlife','Sunset'],'Cliffs, goats and dramatic coastal viewpoints.'],
    ['Brecon Beacons / Pen y Fan',51.8833,-3.4368,'Wales','mountain landscape',['Landscape','Sunrise','Telephoto','Weather'],'Mountain views and dramatic weather; plan safely.'],
    ['Snowdon / Yr Wyddfa',53.0685,-4.0763,'Wales','mountain landscape',['Landscape','Mountain','Weather','Sunrise'],'Classic mountain landscape; conditions change quickly.'],
    ['Llyn Padarn',53.1330,-4.1360,'Wales','lake landscape',['Landscape','Reflections','Sunrise','Telephoto'],'Lake reflections, mountains and Lone Tree compositions.'],
    ['South Stack Lighthouse',53.3061,-4.6996,'Anglesey','coastal lighthouse',['Landscape','Coastal','Wildlife','Sunset'],'Lighthouse, sea cliffs and birdlife.'],
    ['Mam Tor',53.3490,-1.8107,'Peak District','landscape viewpoint',['Landscape','Sunrise','Mist','Telephoto layers'],'Ridge views and misty sunrise conditions.'],
    ['Stanage Edge',53.3463,-1.6375,'Peak District','gritstone edge',['Landscape','Climbing','Sunset','Wide angle'],'Gritstone edge and sweeping Peak District views.'],
    ['Derwent Edge',53.4083,-1.6580,'Peak District','landscape viewpoint',['Landscape','Sunset','Telephoto','Weather'],'Gritstone formations and expansive views.'],
    ['Castleton / Winnats Pass',53.3435,-1.7781,'Peak District','valley landscape',['Landscape','Road','Mist','Telephoto'],'Dramatic valley and road compositions.'],
    ['Buttermere',54.5411,-3.2750,'Lake District','lake landscape',['Landscape','Reflections','Autumn','Sunrise'],'Classic lake, mountain reflections and autumn colour.'],
    ['Derwentwater',54.5845,-3.1384,'Lake District','lake landscape',['Landscape','Reflections','Jetty','Golden hour'],'Lake, jetties, islands and mountain views.'],
    ['Ashness Bridge',54.5588,-3.1267,'Lake District','landscape landmark',['Landscape','Waterfall','Autumn','Classic view'],'Traditional Lake District bridge and viewpoint.'],
    ['Ullswater',54.5770,-2.8690,'Lake District','lake landscape',['Landscape','Reflections','Sunrise','Mist'],'Lake and mountain compositions.'],
    ['Bamburgh Castle',55.6083,-1.7090,'Northumberland','coastal castle',['Landscape','Architecture','Coastal','Sunrise'],'Castle, beach and dramatic dawn light.'],
    ['Holy Island / Lindisfarne',55.6690,-1.7840,'Northumberland','coastal heritage',['Landscape','Architecture','Coastal','Tide planning'],'Castle, causeway and tidal planning essential.'],
    ['Sycamore Gap / Hadrian’s Wall area',55.0034,-2.3734,'Northumberland','historic landscape',['Landscape','Heritage','Telephoto','Sunset'],'Hadrian’s Wall landscape; check current access and conditions.'],
    ['Whitby Abbey',54.4891,-0.6051,'North Yorkshire','coastal heritage',['Architecture','Coastal','Night','Atmosphere'],'Abbey, harbour and atmospheric coastal town scenes.'],
    ['York Minster',53.9623,-1.0819,'York','architecture city',['Architecture','Street','Night','Details'],'Cathedral architecture, streets and details.'],
    ['Glen Coe',56.6826,-5.1023,'Scotland','mountain landscape',['Landscape','Weather','Telephoto','Dramatic light'],'Iconic mountain landscape with fast-changing weather.'],
    ['Buachaille Etive Mòr',56.6469,-4.9014,'Scotland','mountain landscape',['Landscape','Mountain','Sunrise','Waterfall'],'Classic Scottish mountain subject.'],
    ['Eilean Donan Castle',57.2740,-5.5160,'Scotland','castle landscape',['Architecture','Landscape','Blue hour','Reflections'],'Castle, water and mountain context.'],
    ['Old Man of Storr',57.5073,-6.1830,'Isle of Skye','mountain landscape',['Landscape','Sunrise','Weather','Iconic view'],'Iconic Skye viewpoint; plan for wind, rain and early starts.'],
    ['Neist Point Lighthouse',57.4236,-6.7883,'Isle of Skye','coastal lighthouse',['Landscape','Coastal','Sunset','Telephoto'],'Cliff-top lighthouse and dramatic sunset potential.'],
    ['Cairngorms National Park',57.0800,-3.6700,'Scotland','mountain wildlife',['Wildlife','Landscape','Snow','Telephoto'],'Mountains, wildlife and changing conditions.'],
    ['Norfolk Broads',52.6750,1.5500,'Norfolk','wetland landscape',['Wildlife','Landscape','Boats','Mist'],'Wetland, boats, reeds, birds and morning mist.'],
    ['Blakeney Point',52.9690,1.0290,'Norfolk','coastal wildlife',['Wildlife','Coastal','Seals','Telephoto'],'Seals and coastal wildlife; follow access and wildlife guidance.']
  ].map(([name,lat,lng,region,type,goodFor,notes])=>({name,lat,lng,region,type,goodFor,notes,source:'curated'}));
  function custom(){try{return JSON.parse(localStorage.getItem(CUSTOM_KEY)||'[]')}catch{return[]}}
  function saveCustom(list){localStorage.setItem(CUSTOM_KEY,JSON.stringify(list))}
  function allLocs(){return CURATED.concat(custom())}
  function rankLocations(){const u=userLL();const genre=(window.currentRecommendations?.genre?.name||'').toLowerCase();return allLocs().map(loc=>{const dist=u?mi(u.lat,u.lng,loc.lat,loc.lng):null;let score=0;const hay=(loc.name+' '+loc.type+' '+loc.region+' '+(loc.goodFor||[]).join(' ')).toLowerCase();if(genre&&hay.includes(genre))score+=6;if(genre==='landscape'&&/coastal|viewpoint|landscape|mountain|lake|moor/.test(hay))score+=3;if(genre==='wildlife'&&/wildlife|birds|reserve|seals|deer/.test(hay))score+=5;if(genre==='architecture'&&/castle|architecture|heritage|minster|abbey|urban|building/.test(hay))score+=5;if(genre==='street'&&/street|urban|london|brighton|city/.test(hay))score+=5;if(genre==='macro'&&/macro|woodland|flowers|garden/.test(hay))score+=4;if(dist!==null)score+=Math.max(0,5-Math.min(dist/20,5));return{...loc,dist,score}}).sort((a,b)=>b.score-a.score||((a.dist??9999)-(b.dist??9999)));}
  function locCard(loc){return `<div class="suggested-location-card v31-location-card"><div class="v31-loc-head"><h4>${esc(loc.name)}</h4>${loc.source==='custom'?'<span class="v31-user-badge">Your location</span>':''}</div><p class="muted small">${esc(loc.region)} · ${esc(loc.type)}${loc.dist!==null?` · approx. ${loc.dist.toFixed(1)} miles`:''}</p><div class="mini-chips">${(loc.goodFor||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div><p class="small">${esc(loc.notes||'')}</p><div class="suggested-actions"><button class="btn btn-secondary" data-action="navigateApple" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${esc(loc.name)}">Apple Maps</button><button class="btn btn-secondary" data-action="navigateGoogle" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${esc(loc.name)}">Google Maps</button><button class="btn btn-secondary" data-action="navigateWaze" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${esc(loc.name)}">Waze</button>${loc.source==='custom'?`<button class="btn btn-danger" data-action="deleteCustomLocation" data-name="${esc(loc.name)}">Delete</button>`:''}</div></div>`}
  function locSectionHTML(){const ranked=rankLocations();const locs=ranked.slice(0,60);return `<div class="card v31-planner-card" id="v31SuggestedLocations"><h3>${icon('map')} UK Photography Location Suggestions</h3><p class="muted small">Expanded UK-first beta database with ${CURATED.length} curated locations, including Durdle Door. User-added locations are stored privately on this device only.</p><div class="v31-filter-row"><input id="v31LocationSearch" type="search" placeholder="Search locations, e.g. Durdle Door, wildlife, coast, London"><button class="btn btn-secondary" data-action="v31UseGPSWeather">Refresh Live Weather</button></div><div class="suggested-location-list" id="v31LocationList">${locs.map(locCard).join('')}</div><details class="v31-add-location"><summary>Add your own suggested location</summary><div class="v31-add-grid"><input id="v31CustomName" placeholder="Location name"><input id="v31CustomRegion" placeholder="Region"><input id="v31CustomLat" placeholder="Latitude"><input id="v31CustomLng" placeholder="Longitude"><input id="v31CustomType" placeholder="Type, e.g. coastal landscape"><input id="v31CustomTags" placeholder="Tags, e.g. Landscape, Sunset, Wildlife"><textarea id="v31CustomNotes" placeholder="Access, parking, safety or photographic notes"></textarea><div class="v31-add-actions"><button class="btn btn-secondary" data-action="v31FillCurrentGPS">Use current GPS</button><button class="btn btn-primary" data-action="v31AddCustomLocation">Add Location</button></div></div></details></div>`}
  function renderLiveWeatherPanel(data,err){let html='';if(data){const days=(data.daily?.time||[]).map((date,i)=>({date,code:data.daily.weather_code?.[i],max:data.daily.temperature_2m_max?.[i],min:data.daily.temperature_2m_min?.[i],rain:data.daily.precipitation_probability_max?.[i],sunrise:data.daily.sunrise?.[i],sunset:data.daily.sunset?.[i]})).slice(0,5);const cur=data.current||{};html=`<div class="card weather-card" id="v31WeatherPanel"><h3>${icon('light')} Live Weather & Light Planning</h3><p><strong>${cur.temperature_2m??'—'}°C</strong> · ${codeSummary(cur.weather_code)} · Wind ${cur.wind_speed_10m??'—'} km/h</p><div class="weather-days">${days.map(d=>`<div class="weather-day"><strong>${new Date(d.date).toLocaleDateString([], {weekday:'short'})}</strong><span>${codeSummary(d.code)}</span><span>${Math.round(d.min)}–${Math.round(d.max)}°C</span><span>Rain ${d.rain??'—'}%</span><small>Sunrise ${d.sunrise?new Date(d.sunrise).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'—'} · Sunset ${d.sunset?new Date(d.sunset).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'—'}</small></div>`).join('')}</div><p class="muted small">Live forecast from Open‑Meteo. If this does not load, check location permission and mobile signal.</p></div>`}else{html=`<div class="card weather-card" id="v31WeatherPanel"><h3>${icon('light')} Live Weather & Light Planning</h3><p class="muted">Tap “Refresh Live Weather” in Planner after allowing GPS permission.</p>${err?`<p class="warning small">Weather did not load: ${esc(err)}</p>`:''}</div>`}const existing=q('#v31WeatherPanel');if(existing){existing.outerHTML=html}else{const content=q('#plannerTab .content'); if(content)content.insertAdjacentHTML('afterbegin',html)}}
  function getPos(){return new Promise((res,rej)=>{if(!navigator.geolocation)return rej(new Error('Geolocation not supported'));navigator.geolocation.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:12000,maximumAge:120000})})}
  async function refreshWeather(){renderLiveWeatherPanel(null,'Loading live GPS/weather…');try{const pos=await getPos();window.currentPosition=pos;const lat=pos.coords.latitude,lng=pos.coords.longitude;const url=`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,wind_speed_10m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,precipitation_probability_max&timezone=auto&forecast_days=5`;const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error('forecast service '+r.status);const data=await r.json();localStorage.setItem(WEATHER_KEY,JSON.stringify({data,lat,lng,at:Date.now()}));renderLiveWeatherPanel(data);try{renderPlanner()}catch{}}catch(e){renderLiveWeatherPanel(null,e.message||'unknown error')}}
  function rerenderLocationsFromSearch(){const term=(q('#v31LocationSearch')?.value||'').toLowerCase().trim();let locs=rankLocations();if(term)locs=locs.filter(l=>(l.name+' '+l.region+' '+l.type+' '+(l.goodFor||[]).join(' ')+' '+(l.notes||'')).toLowerCase().includes(term));const list=q('#v31LocationList');if(list)list.innerHTML=locs.slice(0,80).map(locCard).join('');try{photoScoutAttachActions(list)}catch{}}
  function fillCurrentGPS(){const u=userLL();if(!u){alert('No GPS captured yet. Tap Refresh Live Weather first, or enter coordinates manually.');return;} q('#v31CustomLat').value=u.lat.toFixed(6); q('#v31CustomLng').value=u.lng.toFixed(6);}
  function addCustom(){const name=q('#v31CustomName')?.value.trim();const lat=Number(q('#v31CustomLat')?.value);const lng=Number(q('#v31CustomLng')?.value);if(!name||!Number.isFinite(lat)||!Number.isFinite(lng)){alert('Please add a location name plus valid latitude and longitude.');return;}const item={name,lat,lng,region:q('#v31CustomRegion')?.value.trim()||'User added',type:q('#v31CustomType')?.value.trim()||'photography location',goodFor:(q('#v31CustomTags')?.value||'').split(',').map(x=>x.trim()).filter(Boolean),notes:q('#v31CustomNotes')?.value.trim()||'User-added Photo Scout location.',source:'custom'};const list=custom();list.unshift(item);saveCustom(list);try{renderPlanner()}catch{};alert('Location added to your private Photo Scout location list on this device.');}
  function deleteCustom(name){if(!confirm('Delete this user-added location from this device?'))return;saveCustom(custom().filter(x=>x.name!==name));try{renderPlanner()}catch{}}
  const priorRun=window.photoScoutRunAction;
  window.photoScoutRunAction=function(action,id,e){const el=e?.target?.closest?.('[data-action]');const lat=Number(el?.dataset?.lat),lng=Number(el?.dataset?.lng),name=el?.dataset?.name||'Photo Scout Location';if(action==='navigateApple'&&Number.isFinite(lat))return openNav(lat,lng,name,'apple'),true;if(action==='navigateGoogle'&&Number.isFinite(lat))return openNav(lat,lng,name,'google'),true;if(action==='navigateWaze'&&Number.isFinite(lat))return openNav(lat,lng,name,'waze'),true;if(action==='v31UseGPSWeather')return refreshWeather(),true;if(action==='v31FillCurrentGPS')return fillCurrentGPS(),true;if(action==='v31AddCustomLocation')return addCustom(),true;if(action==='deleteCustomLocation')return deleteCustom(name),true;if(action==='showWhatsNew'){const dlg=q('#whatsNewDialog'),body=q('#whatsNewContent');if(body){body.innerHTML=`<h2>Photo Scout ${LABEL}</h2><ul class="whats-new-list"><li>Expanded UK photography location database, including Durdle Door.</li><li>Users can add private location suggestions on their own device.</li><li>Planner layout now exposes sat-nav routes more clearly.</li><li>Live weather panel improved with clearer loading/error messages.</li><li>Help/version notes updated from earlier V25 wording.</li></ul><div class="update-actions"><button class="btn btn-primary" data-action="closeWhatsNew">Continue</button></div>`;}try{dlg?.showModal()}catch{}return true;}return priorRun?priorRun(action,id,e):false;};
  const priorPlanner=window.renderPlanner||renderPlanner;
  window.renderPlanner=renderPlanner=function(){priorPlanner();const content=q('#plannerTab .content');if(!content)return;q('#v29SuggestedLocations')?.remove();q('#v31SuggestedLocations')?.remove();q('#v31WeatherPanel')?.remove();let last=null;try{last=JSON.parse(localStorage.getItem(WEATHER_KEY)||'null')}catch{};renderLiveWeatherPanel(last?.data||null);content.insertAdjacentHTML('beforeend',locSectionHTML());q('#v31LocationSearch')?.addEventListener('input',rerenderLocationsFromSearch);try{photoScoutAttachActions(content)}catch{}};
  function patchHelp(){document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=LABEL);const help=q('#helpTab .content')||q('#helpTab');if(!help)return; q('#v31VersionCard')?.remove();help.insertAdjacentHTML('afterbegin',`<div class="card help-section version-card" id="v31VersionCard"><h3>Photo Scout ${LABEL}</h3><p>V31 adds an expanded UK photography location database, private user-added locations, clearer sat-nav buttons and improved live weather feedback.</p><div class="update-actions"><button class="btn btn-secondary" data-action="showWhatsNew">What’s new</button><button class="btn btn-secondary" data-action="clearPhotoScoutCache">Clear cache</button></div></div>`);try{photoScoutAttachActions(help)}catch{}}
  function boot(){document.documentElement.dataset.photoScoutVersion=V;try{localStorage.setItem('photoScout.updateAcknowledgedVersion',V);localStorage.setItem('photoScout.updateDismissedVersion',V);localStorage.setItem('photoScout.lastSeenVersion',V)}catch{};document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=LABEL);setTimeout(()=>{try{renderPlanner()}catch{};patchHelp();try{renderLiveWeatherPanel(JSON.parse(localStorage.getItem(WEATHER_KEY)||'null')?.data||null)}catch{}},700);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
})();

// v32: structured location hub with category sections and expandable user/private location additions.
(function(){
  const V='32.0.0';
  const LABEL='Beta v32';
  const CUSTOM_KEY='photoScout.customSuggestedLocations.v1';
  const OFFICIAL_SUGGESTION_KEY='photoScout.officialLocationSuggestions.v1';
  function q(s,root=document){return root.querySelector(s)}
  function esc(v){return (''+(v??'')).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}
  function enc(v){return encodeURIComponent(v||'Photo Scout Location')}
  function icon(name){try{return iconSVG(name)}catch{return ''}}
  function userLL(){try{if(window.currentPosition?.coords)return{lat:currentPosition.coords.latitude,lng:currentPosition.coords.longitude}}catch{} return null;}
  function miles(a,b,c,d){const R=3958.8,rad=x=>x*Math.PI/180;const dLat=rad(c-a),dLng=rad(d-b);const h=Math.sin(dLat/2)**2+Math.cos(rad(a))*Math.cos(rad(c))*Math.sin(dLng/2)**2;return R*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h));}
  function navURL(lat,lng,name,app){if(app==='apple')return`https://maps.apple.com/?daddr=${lat},${lng}&q=${enc(name)}`; if(app==='google')return`https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving`; if(app==='waze')return`https://waze.com/ul?ll=${lat},${lng}&navigate=yes`; return`https://www.google.com/maps/search/?api=1&query=${lat},${lng}`;}
  function openNav(lat,lng,name,app){window.open(navURL(Number(lat),Number(lng),name,app),'_blank','noopener')}
  function custom(){try{return JSON.parse(localStorage.getItem(CUSTOM_KEY)||'[]')}catch{return[]}}
  function saveCustom(list){localStorage.setItem(CUSTOM_KEY,JSON.stringify(list))}
  function officialSuggestions(){try{return JSON.parse(localStorage.getItem(OFFICIAL_SUGGESTION_KEY)||'[]')}catch{return[]}}
  function saveOfficialSuggestions(list){localStorage.setItem(OFFICIAL_SUGGESTION_KEY,JSON.stringify(list))}

  const LOCS=[
    {name:'Durdle Door',lat:50.6211,lng:-2.2768,region:'Dorset',category:'Coastal',type:'coastal landscape',goodFor:['Landscape','Coastal','Sunrise','Astro','Long exposure'],notes:'Iconic Jurassic Coast arch. Strong sunrise/blue hour potential; check cliff safety, parking and access.'},
    {name:'Lulworth Cove',lat:50.6185,lng:-2.2478,region:'Dorset',category:'Coastal',type:'coastal landscape',goodFor:['Landscape','Coastal','Golden hour','Seascape'],notes:'Curved cove and cliff paths close to Durdle Door.'},
    {name:'Old Harry Rocks',lat:50.6423,lng:-1.9235,region:'Dorset',category:'Coastal',type:'chalk stacks',goodFor:['Landscape','Coastal','Sunrise','Wide angle'],notes:'Chalk stacks and sweeping coast; excellent in directional early light.'},
    {name:'Portland Bill Lighthouse',lat:50.5145,lng:-2.4576,region:'Dorset',category:'Coastal',type:'lighthouse coast',goodFor:['Landscape','Long exposure','Night','Storm light'],notes:'Lighthouse, rocks and waves. Use care in rough weather.'},
    {name:'Seven Sisters & Cuckmere Haven',lat:50.7607,lng:0.1480,region:'East Sussex',category:'National Trust',type:'coastal cliffs',goodFor:['Landscape','Coastal','Golden hour','Long exposure'],notes:'Classic chalk-cliff composition and river mouth views; avoid cliff edges.'},
    {name:'Birling Gap',lat:50.7429,lng:0.2018,region:'East Sussex',category:'National Trust',type:'coastal cliffs',goodFor:['Landscape','Coastal','Sunset','Minimalist'],notes:'National Trust coastal location with dramatic cliff and beach access; check tide and cliff safety.'},
    {name:'Bodiam Castle',lat:51.0023,lng:0.5435,region:'East Sussex',category:'National Trust',type:'moated castle',goodFor:['Architecture','Landscape','Reflections','Mist'],notes:'Moated castle with strong reflection opportunities, especially in calm/misty conditions.'},
    {name:'Corfe Castle',lat:50.6395,lng:-2.0566,region:'Dorset',category:'National Trust',type:'ruins landscape',goodFor:['Architecture','Landscape','Mist','Sunrise'],notes:'Ruins, village and surrounding hills; strong dawn/mist potential.'},
    {name:'St Michael’s Mount',lat:50.1179,lng:-5.4775,region:'Cornwall',category:'National Trust',type:'coastal heritage',goodFor:['Landscape','Architecture','Coastal','Blue hour'],notes:'Island castle and causeway; tide planning is important.'},
    {name:'Fountains Abbey & Studley Royal',lat:54.1096,lng:-1.5820,region:'North Yorkshire',category:'National Trust',type:'abbey/landscape',goodFor:['Architecture','Landscape','Reflections','Autumn'],notes:'Ruins, water garden and architectural details.'},
    {name:'Glastonbury Tor',lat:51.1445,lng:-2.6992,region:'Somerset',category:'Landscape',type:'landmark viewpoint',goodFor:['Landscape','Sunrise','Mist','Telephoto'],notes:'Iconic hilltop tower, mist and layered Somerset views.'},
    {name:'Ditchling Beacon',lat:50.9015,lng:-0.1077,region:'West Sussex',category:'Landscape',type:'South Downs viewpoint',goodFor:['Landscape','Sunrise','Telephoto layers'],notes:'High viewpoint across the South Downs.'},
    {name:'Mam Tor',lat:53.3490,lng:-1.8107,region:'Peak District',category:'Landscape',type:'ridge viewpoint',goodFor:['Landscape','Sunrise','Mist','Telephoto'],notes:'Ridge views and classic Peak District misty sunrise conditions.'},
    {name:'Buttermere',lat:54.5411,lng:-3.2750,region:'Lake District',category:'Landscape',type:'lake landscape',goodFor:['Landscape','Reflections','Autumn','Sunrise'],notes:'Classic lake and mountain reflections.'},
    {name:'Derwentwater',lat:54.5845,lng:-3.1384,region:'Lake District',category:'Landscape',type:'lake landscape',goodFor:['Landscape','Jetty','Reflections','Golden hour'],notes:'Jetties, islands and mountain views.'},
    {name:'Snowdon / Yr Wyddfa',lat:53.0685,lng:-4.0763,region:'Wales',category:'Landscape',type:'mountain landscape',goodFor:['Landscape','Mountain','Weather','Sunrise'],notes:'Mountain landscape; plan safely and check conditions.'},
    {name:'Glen Coe',lat:56.6826,lng:-5.1023,region:'Scotland',category:'Landscape',type:'mountain landscape',goodFor:['Landscape','Weather','Telephoto','Dramatic light'],notes:'Iconic mountain landscape with fast-changing conditions.'},
    {name:'Ashdown Forest',lat:51.0660,lng:0.0330,region:'East Sussex',category:'Woodland',type:'woodland/heathland',goodFor:['Woodland','Landscape','Wildlife','Autumn'],notes:'Heathland, woodland, mist and seasonal detail work.'},
    {name:'New Forest National Park',lat:50.8750,lng:-1.6320,region:'Hampshire',category:'Woodland',type:'woodland/heathland',goodFor:['Wildlife','Woodland','Landscape','Autumn'],notes:'Ponies, ancient woodland, mist and heathland.'},
    {name:'Wistman’s Wood',lat:50.5787,lng:-3.9603,region:'Devon',category:'Woodland',type:'ancient woodland',goodFor:['Woodland','Macro','Mist','Atmosphere'],notes:'Mossy ancient woodland; tread carefully and protect the habitat.'},
    {name:'RSPB Pulborough Brooks',lat:50.9519,lng:-0.5132,region:'West Sussex',category:'Wildlife',type:'wetland reserve',goodFor:['Wildlife','Birds','Telephoto','Dawn'],notes:'Bird hides and wetland wildlife; check opening hours.'},
    {name:'Rye Harbour Nature Reserve',lat:50.9359,lng:0.7582,region:'East Sussex',category:'Wildlife',type:'coastal reserve',goodFor:['Wildlife','Birds','Coastal','Big skies'],notes:'Birds, wetland, shingle and big-sky compositions.'},
    {name:'Richmond Park',lat:51.4440,lng:-0.2740,region:'London',category:'Wildlife',type:'deer park',goodFor:['Wildlife','Landscape','Mist','Telephoto'],notes:'Deer, misty mornings and open parkland; keep safe distance.'},
    {name:'London South Bank',lat:51.5066,lng:-0.1163,region:'London',category:'Urban / Street',type:'urban/street',goodFor:['Street','Architecture','Night','Reflections'],notes:'Street, bridges, people, blue-hour skyline and reflections.'},
    {name:'Brighton Palace Pier',lat:50.8195,lng:-0.1368,region:'East Sussex',category:'Urban / Street',type:'urban coastal',goodFor:['Street','Architecture','Night','Seascape'],notes:'Pier, seafront, night photography and long-exposure opportunities.'},
    {name:'Tower Bridge',lat:51.5055,lng:-0.0754,region:'London',category:'Architecture',type:'landmark architecture',goodFor:['Architecture','Blue hour','Night','Reflections'],notes:'Classic London landmark with strong blue-hour potential.'},
    {name:'Canary Wharf',lat:51.5054,lng:-0.0235,region:'London',category:'Architecture',type:'modern architecture',goodFor:['Architecture','Street','Night','Reflections'],notes:'Glass, symmetry, reflections and urban night photography.'},
    {name:'Sgwd yr Eira',lat:51.7601,lng:-3.5902,region:'Wales',category:'Waterfalls & Rivers',type:'waterfall',goodFor:['Waterfall','Landscape','Long exposure','Woodland'],notes:'Waterfall and woodland; tripod/ND filter useful.'},
    {name:'Ashness Bridge',lat:54.5588,lng:-3.1267,region:'Lake District',category:'Waterfalls & Rivers',type:'bridge/waterfall',goodFor:['Landscape','Waterfall','Autumn','Classic view'],notes:'Classic Lake District bridge and nearby viewpoint.'},
    {name:'Kielder Forest Dark Sky Park',lat:55.2330,lng:-2.5800,region:'Northumberland',category:'Astro',type:'dark sky',goodFor:['Astro','Night','Landscape','Milky Way'],notes:'Dark-sky area; check moon phase, cloud and access.'},
    {name:'Dartmoor Haytor',lat:50.5808,lng:-3.7555,region:'Devon',category:'Astro',type:'moorland/tor',goodFor:['Astro','Landscape','Sunrise','Telephoto'],notes:'Granite tor, moorland layers and night-sky potential.'}
  ];
  const CATEGORIES=['National Trust','Coastal','Landscape','Wildlife','Woodland','Architecture','Urban / Street','Astro','Waterfalls & Rivers','User Added'];
  function allLocations(){return LOCS.concat(custom().map(x=>({...x,category:x.category||'User Added',source:'custom'})))}
  function rankedList(category='All',term=''){
    const u=userLL(); const g=(window.currentRecommendations?.genre?.name||'').toLowerCase(); term=term.toLowerCase().trim();
    return allLocations().filter(l=>(category==='All'||l.category===category)&&(!term||(l.name+' '+l.region+' '+l.category+' '+l.type+' '+(l.goodFor||[]).join(' ')+' '+l.notes).toLowerCase().includes(term))).map(l=>{
      let score=0; const hay=(l.category+' '+l.type+' '+(l.goodFor||[]).join(' ')).toLowerCase();
      if(g&&hay.includes(g))score+=6; if(g==='landscape'&&/(coastal|landscape|mountain|lake|waterfall|astro)/.test(hay))score+=3; if(g==='wildlife'&&/(wildlife|birds|reserve|deer)/.test(hay))score+=4; if(g==='architecture'&&/(architecture|castle|heritage|urban|abbey)/.test(hay))score+=4; if(g==='street'&&/(street|urban|london|brighton)/.test(hay))score+=4; if(g==='macro'&&/(woodland|macro|flowers|waterfall)/.test(hay))score+=3;
      const dist=u?miles(u.lat,u.lng,l.lat,l.lng):null; if(dist!==null)score+=Math.max(0,4-Math.min(dist/35,4));
      return {...l,score,dist};
    }).sort((a,b)=>b.score-a.score||String(a.name).localeCompare(String(b.name)));
  }
  function locCard(l){return `<div class="suggested-location-card v32-location-card" data-cat="${esc(l.category)}"><div class="v32-location-heading"><div><h4>${esc(l.name)}</h4><p class="muted small">${esc(l.category)} · ${esc(l.region)} · ${esc(l.type)}${l.dist!==null&&l.dist!==undefined?` · approx. ${l.dist.toFixed(1)} miles`:''}</p></div>${l.source==='custom'?'<span class="user-pill">User added</span>':'<span class="user-pill curated">Curated</span>'}</div><div class="mini-chips">${(l.goodFor||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div><p class="small">${esc(l.notes||'')}</p><div class="suggested-actions"><button class="btn btn-secondary" data-action="navigateApple" data-lat="${l.lat}" data-lng="${l.lng}" data-name="${esc(l.name)}">Apple Maps</button><button class="btn btn-secondary" data-action="navigateGoogle" data-lat="${l.lat}" data-lng="${l.lng}" data-name="${esc(l.name)}">Google Maps</button><button class="btn btn-secondary" data-action="navigateWaze" data-lat="${l.lat}" data-lng="${l.lng}" data-name="${esc(l.name)}">Waze</button>${l.source==='custom'?`<button class="btn btn-danger" data-action="v32DeleteCustomLocation" data-name="${esc(l.name)}">Delete</button>`:''}</div></div>`}
  function renderList(){const cat=q('#v32CategorySelect')?.value||'All'; const term=q('#v32LocationSearch')?.value||''; const list=rankedList(cat,term); const el=q('#v32LocationList'); if(el){el.innerHTML=list.slice(0,120).map(locCard).join('')||'<p class="muted">No matching locations found.</p>'; try{photoScoutAttachActions(el)}catch{}} const count=q('#v32LocationCount'); if(count)count.textContent=`${list.length} shown · ${LOCS.length} curated · ${custom().length} user added`;}
  function hubHTML(){return `<div class="card v32-location-hub" id="v32LocationHub"><h3>${icon('map')} Suggested Photography Locations</h3><p class="muted small">V32 introduces a scalable category structure. Locations can be added later by Anthony as official curated entries, while users can add private locations on their own device during beta testing.</p><div class="v32-filter-row"><select id="v32CategorySelect"><option value="All">All categories</option>${CATEGORIES.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select><input id="v32LocationSearch" type="search" placeholder="Search e.g. Durdle Door, National Trust, astro, wildlife"><button class="btn btn-secondary" data-action="v31UseGPSWeather">Refresh GPS/weather</button></div><p class="muted small" id="v32LocationCount"></p><div class="v32-category-pills">${CATEGORIES.map(c=>`<button class="v32-cat-pill" data-action="v32SelectCategory" data-category="${esc(c)}">${esc(c)}</button>`).join('')}</div><div class="suggested-location-list" id="v32LocationList"></div><details class="v31-add-location v32-add-location"><summary>Add your own private location</summary><p class="muted small">Beta note: user-added locations are private to this device. Community/official submissions should be reviewed before becoming part of the curated public database.</p><div class="v31-add-grid"><input id="v32CustomName" placeholder="Location name"><select id="v32CustomCategory">${CATEGORIES.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select><input id="v32CustomRegion" placeholder="Region"><input id="v32CustomLat" placeholder="Latitude"><input id="v32CustomLng" placeholder="Longitude"><input id="v32CustomType" placeholder="Type, e.g. coastal landscape"><input id="v32CustomTags" placeholder="Tags, e.g. Landscape, Sunset, Wildlife"><textarea id="v32CustomNotes" placeholder="Access, parking, safety or photographic notes"></textarea><div class="v31-add-actions"><button class="btn btn-secondary" data-action="v32FillCurrentGPS">Use current GPS</button><button class="btn btn-primary" data-action="v32AddCustomLocation">Add Private Location</button></div></div></details><details class="v31-add-location v32-official-suggest"><summary>Suggest a location for the official curated database</summary><p class="muted small">This records a suggested official addition locally so it can be reviewed later. In a future cloud/native version this could be submitted for moderation.</p><div class="v31-add-grid"><input id="v32SuggestName" placeholder="Suggested location name"><select id="v32SuggestCategory">${CATEGORIES.filter(c=>c!=='User Added').map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select><input id="v32SuggestRegion" placeholder="Region"><textarea id="v32SuggestReason" placeholder="Why should this be included? Best genre, access notes, parking, safety, seasonal advice."></textarea><button class="btn btn-primary" data-action="v32AddOfficialSuggestion">Save Suggestion for Review</button></div><div id="v32OfficialSuggestionList"></div></details></div>`}
  function fillGPS(){const u=userLL(); if(!u){alert('No GPS captured yet. Use Refresh GPS/weather first, or enter coordinates manually.'); return;} q('#v32CustomLat').value=u.lat.toFixed(6); q('#v32CustomLng').value=u.lng.toFixed(6);}
  function addCustom(){const name=q('#v32CustomName')?.value.trim(); const lat=Number(q('#v32CustomLat')?.value); const lng=Number(q('#v32CustomLng')?.value); if(!name||!Number.isFinite(lat)||!Number.isFinite(lng)){alert('Please add a name plus valid latitude and longitude.');return;} const item={name,lat,lng,category:q('#v32CustomCategory')?.value||'User Added',region:q('#v32CustomRegion')?.value.trim()||'User added',type:q('#v32CustomType')?.value.trim()||'photography location',goodFor:(q('#v32CustomTags')?.value||'').split(',').map(x=>x.trim()).filter(Boolean),notes:q('#v32CustomNotes')?.value.trim()||'User-added Photo Scout location.',source:'custom'}; const list=custom(); list.unshift(item); saveCustom(list); alert('Private location added on this device.'); renderList();}
  function delCustom(name){if(!confirm('Delete this private location from this device?'))return; saveCustom(custom().filter(x=>x.name!==name)); renderList();}
  function renderOfficialSuggestions(){const el=q('#v32OfficialSuggestionList'); if(!el)return; const list=officialSuggestions(); el.innerHTML=list.length?`<div class="mini-chips">${list.map(x=>`<span>${esc(x.name)} · ${esc(x.category)}</span>`).join('')}</div>`:'<p class="muted small">No official suggestions saved yet.</p>';}
  function addOfficialSuggestion(){const name=q('#v32SuggestName')?.value.trim(); if(!name){alert('Please add a location name.');return;} const item={name,category:q('#v32SuggestCategory')?.value||'Landscape',region:q('#v32SuggestRegion')?.value.trim()||'',reason:q('#v32SuggestReason')?.value.trim()||'',date:new Date().toISOString()}; const list=officialSuggestions(); list.unshift(item); saveOfficialSuggestions(list); q('#v32SuggestName').value=''; q('#v32SuggestReason').value=''; alert('Suggestion saved locally for review.'); renderOfficialSuggestions();}
  const priorRun=window.photoScoutRunAction;
  window.photoScoutRunAction=function(action,id,e){const el=e?.target?.closest?.('[data-action]'); const lat=Number(el?.dataset?.lat), lng=Number(el?.dataset?.lng), name=el?.dataset?.name||'Photo Scout Location'; if(action==='navigateApple'&&Number.isFinite(lat))return openNav(lat,lng,name,'apple'),true; if(action==='navigateGoogle'&&Number.isFinite(lat))return openNav(lat,lng,name,'google'),true; if(action==='navigateWaze'&&Number.isFinite(lat))return openNav(lat,lng,name,'waze'),true; if(action==='v32SelectCategory'){const c=el?.dataset?.category||'All'; const sel=q('#v32CategorySelect'); if(sel)sel.value=c; renderList(); return true;} if(action==='v32FillCurrentGPS')return fillGPS(),true; if(action==='v32AddCustomLocation')return addCustom(),true; if(action==='v32DeleteCustomLocation')return delCustom(name),true; if(action==='v32AddOfficialSuggestion')return addOfficialSuggestion(),true; if(action==='showWhatsNew'){const dlg=q('#whatsNewDialog'),body=q('#whatsNewContent'); if(body){body.innerHTML=`<h2>Photo Scout ${LABEL}</h2><ul class="whats-new-list"><li>Suggested Locations now use a scalable category structure.</li><li>National Trust now has its own subsection.</li><li>Users can add private locations to their own device.</li><li>Potential official database suggestions can be saved locally for later review.</li><li>Sat-nav buttons remain available for Apple Maps, Google Maps and Waze.</li></ul><div class="update-actions"><button class="btn btn-primary" data-action="closeWhatsNew">Continue</button></div>`;} try{dlg?.showModal()}catch{} return true;} return priorRun?priorRun(action,id,e):false;};
  const priorPlanner=window.renderPlanner||renderPlanner;
  window.renderPlanner=renderPlanner=function(){priorPlanner(); const content=q('#plannerTab .content'); if(!content)return; q('#v29SuggestedLocations')?.remove(); q('#v31SuggestedLocations')?.remove(); q('#v32LocationHub')?.remove(); content.insertAdjacentHTML('beforeend',hubHTML()); q('#v32CategorySelect')?.addEventListener('change',renderList); q('#v32LocationSearch')?.addEventListener('input',renderList); renderList(); renderOfficialSuggestions(); try{photoScoutAttachActions(content)}catch{} };
  function patchHelp(){document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=LABEL); const help=q('#helpTab .content')||q('#helpTab'); if(!help)return; q('#v31VersionCard')?.remove(); q('#v32VersionCard')?.remove(); help.insertAdjacentHTML('afterbegin',`<div class="card help-section version-card" id="v32VersionCard"><h3>Photo Scout ${LABEL}</h3><p>V32 adds the new structured location database: National Trust, Coastal, Landscape, Wildlife, Woodland, Architecture, Urban / Street, Astro, Waterfalls & Rivers and User Added.</p><p class="muted small">Locations can be expanded later as curated official entries, or added privately by users during beta testing.</p><div class="update-actions"><button class="btn btn-secondary" data-action="showWhatsNew">What’s new</button><button class="btn btn-secondary" data-action="clearPhotoScoutCache">Clear cache</button></div></div>`); try{photoScoutAttachActions(help)}catch{} }
  function boot(){document.documentElement.dataset.photoScoutVersion=V; try{localStorage.setItem('photoScout.updateAcknowledgedVersion',V);localStorage.setItem('photoScout.updateDismissedVersion',V);localStorage.setItem('photoScout.lastSeenVersion',V)}catch{} document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=LABEL); setTimeout(()=>{try{renderPlanner()}catch{}; patchHelp();},800);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot); else boot();
})();


/* Photo Scout V34 — curated location additions + rating system */
(function(){
  const VERSION='34.0.0';
  const LABEL='Beta v34';
  const RATING_KEY='photoScout.locationRatings.v1';
  const V34_LOCS=[
    {name:'Halnaker Tree Tunnel / Halnaker Windmill',lat:50.8589,lng:-0.7007,region:'West Sussex',category:'Woodland',type:'woodland leading lines / historic windmill',goodFor:['Woodland','Landscape','Leading lines','Autumn','Golden hour','Blue hour'],notes:'Natural Lands / public footpath route leading up to Halnaker Windmill. Excellent tree tunnel leading lines and elevated South Downs views. Parking is limited near the A285/lay-by area; roadside spaces can become busy. Path may be muddy after rain.'},
    {name:'Bosham Harbour',lat:50.8309,lng:-0.8546,region:'West Sussex',category:'Coastal',type:'photogenic harbour / waterscape',goodFor:['Landscape','Waterscape','Harbour','Reflections','Golden hour','Blue hour'],notes:'Historic photogenic harbour village with boats, moorings, tidal reflections and strong foreground opportunities. Tide awareness recommended; village parking and narrow roads can become busy at weekends and in summer.'},
    {name:'Itchenor',lat:50.8033,lng:-0.8639,region:'West Sussex',category:'Coastal',type:'waterscape / sailing harbour',goodFor:['Landscape','Waterscape','Sailing','Reflections','Long exposure','Golden hour'],notes:'South Coast waterscape location with tidal variation, harbour scenes and sailing subjects. Best in calm wind for reflections or soft light for minimalist waterscapes. Parking near harbour/marina areas can be busy.'},
    {name:'Keere Street, Lewes',lat:50.8711,lng:-0.0083,region:'East Sussex',category:'Urban / Street',type:'historic cobbled street',goodFor:['Street','Architecture','Leading lines','Black and white','Blue hour','Wet reflections'],notes:'Historic cobbled street with strong leading lines, textures and atmospheric period architecture. Parking is within Lewes town; short walk required. Works well early morning, blue hour or after rain.'},
    {name:'Winkworth Arboretum',lat:51.1862,lng:-0.5956,region:'Surrey',category:'National Trust',type:'arboretum / autumn colour / boathouse',goodFor:['National Trust','Woodland','Landscape','Autumn colour','Reflections','Boathouse'],notes:'National Trust arboretum famous for autumn colour and the iconic boathouse view. Excellent lake reflections in calm conditions. Onsite parking; can become very busy during peak autumn weekends. Uneven woodland paths in places.'}
  ];
  ...
  function saveRatings(r){try{localStorage.setItem(RATING_KEY,JSON.stringify(r))}catch{}}
  function ratingFor(name){return Number(loadRatings()[name]||0)}
  function stars(name){const v=ratingFor(name);return `<div class="v34-rating" data-rating-name="${esc(name)}" aria-label="Rating for ${esc(name)}">${[1,2,3,4,5].map(n=>`<button class="v34-star ${n<=v?'active':''}" data-action="v34RateLocation" data-name="${esc(name)}" data-rating="${n}" title="Rate ${n} stars">★</button>`).join('')}<span class="v34-rating-label">${v?`${v}/5`:'Rate'}</span></div>`}
  function openNav(lat,lng,name,mode){
    const enc=encodeURIComponent(name||'Photo Scout location');
    let url=`https://www.google.com/maps/search/?api=1&query=${lat},${lng}`;
    if(mode==='apple')url=`https://maps.apple.com/?ll=${lat},${lng}&q=${enc}`;
    if(mode==='waze')url=`https://waze.com/ul?ll=${lat},${lng}&navigate=yes`;
    window.open(url,'_blank');
  }
  function locCard(loc){return `<div class="suggested-location-card v34-location-card"><div class="v34-loc-title"><h4>${esc(loc.name)}</h4><span class="v34-cat">${esc(loc.category)}</span></div><p class="muted small">${esc(loc.region)} · ${esc(loc.type)}</p><div class="mini-chips">${(loc.goodFor||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div><p class="small">${esc(loc.notes)}</p>${stars(loc.name)}<div class="suggested-actions"><button class="btn btn-secondary" data-action="navigateApple" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${esc(loc.name)}">Apple Maps</button><button class="btn btn-secondary" data-action="navigateGoogle" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${esc(loc.name)}">Google Maps</button><button class="btn btn-secondary" data-action="navigateWaze" data-lat="${loc.lat}" data-lng="${loc.lng}" data-name="${esc(loc.name)}">Waze</button></div></div>`}
  function additionsHTML(){return `<div class="card v34-additions" id="v34CuratedAdditions"><h3>New Curated Location Additions</h3><p class="muted small">V34 adds photographer-curated locations supplied during beta testing. Ratings are stored privately on this device.</p><div class="suggested-location-list">${V34_LOCS.map(locCard).join('')}</div></div>`}
  function refreshRatings(){qa('.v34-rating').forEach(box=>{const name=box.dataset.ratingName;box.outerHTML=stars(name);});try{photoScoutAttachActions(document)}catch{}}
  const previousRun=window.photoScoutRunAction;
  window.photoScoutRunAction=function(action,id,e){
    const el=e?.target?.closest?.('[data-action]');
    if(action==='v34RateLocation'){
      const name=el?.dataset?.name; const val=Number(el?.dataset?.rating||0);
      if(name&&val){const r=loadRatings(); r[name]=val; saveRatings(r); refreshRatings();}
      return true;
    }
    const lat=Number(el?.dataset?.lat),lng=Number(el?.dataset?.lng),name=el?.dataset?.name||'Photo Scout Location';
    if(action==='navigateApple'&&Number.isFinite(lat))return openNav(lat,lng,name,'apple'),true;
    if(action==='navigateGoogle'&&Number.isFinite(lat))return openNav(lat,lng,name,'google'),true;
    if(action==='navigateWaze'&&Number.isFinite(lat))return openNav(lat,lng,name,'waze'),true;
    if(action==='showWhatsNew'){
      const dlg=q('#whatsNewDialog'),body=q('#whatsNewContent');
      if(body){body.innerHTML=`<h2>Photo Scout ${LABEL}</h2><ul class="whats-new-list"><li>Added Halnaker Tree Tunnel / Halnaker Windmill with parking/access notes.</li><li>Added Bosham Harbour, Itchenor and Keere Street, Lewes.</li><li>Added National Trust site Winkworth Arboretum with autumn/boathouse guidance.</li><li>Added 1–5 star location rating system stored privately on-device.</li><li>Sat-nav links retained for Apple Maps, Google Maps and Waze.</li></ul><div class="update-actions"><button class="btn btn-primary" data-action="closeWhatsNew">Continue</button></div>`;}
      try{dlg?.showModal()}catch{}
      return true;
    }
    return previousRun?previousRun(action,id,e):false;
  };
  const previousPlanner=window.renderPlanner;
  window.renderPlanner=function(){
    if(previousPlanner) previousPlanner();
    const content=q('#plannerTab .content'); if(!content)return;
    q('#v34CuratedAdditions')?.remove();
    const hub=q('#v32LocationHub')||q('#v31SuggestedLocations');
    if(hub) hub.insertAdjacentHTML('beforebegin',additionsHTML()); else content.insertAdjacentHTML('beforeend',additionsHTML());
    try{photoScoutAttachActions(content)}catch{}
  };
  function patchHelp(){
    document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=LABEL);
    const help=q('#helpTab .content')||q('#helpTab'); if(!help)return;
    q('#v31VersionCard')?.remove(); q('#v32VersionCard')?.remove(); q('#v34VersionCard')?.remove();
    help.insertAdjacentHTML('afterbegin',`<div class="card help-section version-card" id="v34VersionCard"><h3>Photo Scout ${LABEL}</h3><p>V34 expands the curated location database with Halnaker Tree Tunnel / Halnaker Windmill, Bosham Harbour, Itchenor, Keere Street Lewes and Winkworth Arboretum.</p><p class="muted small">A private 1–5 star rating system has also been added so testers can rate recommended locations on their own device.</p><div class="update-actions"><button class="btn btn-secondary" data-action="showWhatsNew">What’s new</button><button class="btn btn-secondary" data-action="clearPhotoScoutCache">Clear cache</button></div></div>`);
    try{photoScoutAttachActions(help)}catch{}
  }
  function boot(){
    try{localStorage.setItem('photoScout.updateAcknowledgedVersion',VERSION);localStorage.setItem('photoScout.updateDismissedVersion',VERSION);localStorage.setItem('photoScout.lastSeenVersion',VERSION)}catch{}
    document.documentElement.dataset.photoScoutVersion=VERSION;
    document.querySelectorAll('#appVersionPill,.photo-scout-version').forEach(el=>el.textContent=LABEL);
    setTimeout(()=>{try{if(q('#plannerTab.active'))window.renderPlanner()}catch{}; patchHelp();},700);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot); else boot();
})();
