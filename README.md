...
## v23 Advanced Beta update
- Consolidates previous beta fixes and UI polish.
- Adds a live horizon/level guide toggle for landscape and architecture scouting.
- Adds composition guidance cards after analysis and inside saved location details.
- Adds saved-location ratings, tags and favourite markers.
- Adds optional short voice scout notes stored locally with a saved location where microphone recording is supported.
- Adds an offline scout cache note and clearer field-use guidance.
- Adds experimental return-condition matching using saved light/weather notes and current weather where available.
- Keeps the stronger black grid, improved splash progress, robust Save/Share/Retake/Feedback handling and local subject detection.

After uploading to GitHub Pages, test using `?v=22` at the end of the URL and remove/re-add the iPhone home-screen icon if needed.

## V25 UI Refinement Polish
- Compact professional manual-override genre grid
- Smaller SVG genre icons with tighter card spacing
- Refined selected-state styling
- Alternative matches converted to pill layout
- Stronger Apply selected genre button


## v26 Update & Cache Management

Photo Scout now includes automatic update/version detection for beta testing. Upload the full folder structure to GitHub Pages as before. Testers should normally be able to reopen the installed PWA and receive the latest cached files automatically. The Help screen now shows the current beta version, a What’s New panel, a Check for Updates button and a Clear Cache button.

If a tester still sees an old build, ask them to close and reopen the app. As a fallback, the URL query format still works, for example `?v=26`.


## V30 update refinement
- Repeated update banner issue refined.
- Dismiss until next version supported.
- Version now reports Beta v30.
- Test after upload with `?v=30`.
